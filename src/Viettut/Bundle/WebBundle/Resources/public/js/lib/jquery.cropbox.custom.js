...
(function() {
	...
	function factory($) {
		...
		Crop.prototype = {
			init: function () {
				...
				var defaultControls = $('<div/>', { 'class' : 'cropControls' })
							.append($('<span>'+ this.options.cropControls +'</span>'))
							.append($('<a/>', { 'class' : 'cropZoomIn' }).attr('title','이미지 확대').on('click', $.proxy(this.zoomIn, this)))
							.append($('<a/>', { 'class' : 'cropZoomOut' }).attr('title','이미지 축소').on('click', $.proxy(this.zoomOut, this)));
				...
				if (typeof $.fn.hammer === 'function' || typeof Hammer !== 'undefined') {
					...
					}).on("mousedown",function(e){	// by echos(2014-08-18)
						// console.log('mousedown');
						$(".cropImage").addClass('grabbing');
					}).on("mouseup",function(e){	// by echos(2014-08-18)
						// console.log('mouseup');
						$(".cropImage").removeClass('grabbing');
		...
		$.fn[pluginName].defaultOptions = {
			...
			controls: null,
			showControls: 'auto',
			cropControls: '마우스로 끌어서 보여지는 화면 그대로 편집됩니다.'
		};
	}
