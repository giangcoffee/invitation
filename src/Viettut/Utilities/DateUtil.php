...
trait DateUtil
{
    protected function jdFromDate($day, $month, $year)
    {
        $a = floor((14 - $month) / 12);
        $y = $year + 4800 - $a;
        $m = $month + 12 * $a - 3;
    ...
    protected function jdToDate($jd)
    {
    ...
    protected function getNewMoonDay($k, $timeZone)
    {
    ...
    protected function getSunLongitude($jdn, $timeZone)
    {
    ...
    protected function getLunarMonth11($yy, $timeZone)
    {
    ...
    protected function getLeapMonthOffset($a11, $timeZone)
    {
    ...
    public function getDateString(DateTime $date)
    {
        setlocale(LC_TIME, 'vi_VN');
        return strftime("%A, %e %B Năm %Y, hồi %k:%M %p", $date->getTimestamp());
    }

    public function getLunarDateString(DateTime $date)
    {
        $lunarResult = $this->convertSolar2Lunar(intval($date->format('d')), intval($date->format('m')), intval($date->format('Y')), 7.0);
        return sprintf('Ngày %s tháng %s năm %s âm lịch', $lunarResult[0], $lunarResult[1], $lunarResult[2]);
    }
}